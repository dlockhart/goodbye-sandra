...
import { formatDateInISO, getDateFromDateObj, getDateFromISODate, getDateTimeDescriptorShared, getToday } from '@brightspace-ui/core/helpers/dateTime.js';
...
class Calendar extends LocalizeStaticMixin(RtlMixin(LitElement)) {
	...
	static get properties() {
		return {
			eventDates: { type: Array, attribute: false },
	...
	static get styles() {
		return [bodySmallStyles, heading4Styles, offscreenStyles, css`
			...
			.d2l-calendar-date:focus d2l-navigation-notification-icon {
				top: -6px;
				right: calc(-100% + 7px);
			}
	...
	render() {
		...
		const dayRows = dates.map((week) => {
			const weekHtml = week.map((day) => {
				...
				const notificationDot = this.eventDates && this.eventDates.find(eventDate => eventDate === (formatDate(day, {format: 'medium'})))
					? html`<d2l-navigation-notification-icon></d2l-navigation-notification-icon>`
					: null;
				...
				return html`
					<td
						...
						<div aria-label="${description}" class="${classMap(classes)}" role="button">${date}${notificationDot}</div>
					</td>`;
			});
...
customElements.define('goodbye-sandra-calendar', Calendar);
